 ...
 *
 * 这是 monolithic-multi-module 架构的核心：
 * - UserModule 监听 OrderCreatedEvent（契约定义在 @mmm/shared-kernel）
 * - OrderModule 发布事件后，不知道谁在监听
 * - 两个模块通过 EventBus 解耦，未来可以独立抽离为微服务
 *
 * @EventsHandler 自动注册到 CQRS EventBus，
 * 当 OrderApplicationServiceImpl 发布 OrderCreatedEvent 时被调用。
 */

import { Injectable, Inject, Logger } from "@nestjs/common";
import { EventsHandler, IEventHandler } from "@nestjs/cqrs";
import { OrderCreatedEvent } from "@mmm/shared-kernel";
