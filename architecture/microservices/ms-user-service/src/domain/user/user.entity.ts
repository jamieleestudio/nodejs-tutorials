/** User 领域实体 */
import { Entity } from "@ms/shared-kernel";
...
export class User extends Entity<string> {
  ...
  static create(params: { id?: string; email: string; name: string }): User {
    const now = new Date();
    return new User(params.id ?? crypto.randomUUID(), new Email(params.email), User.validateName(params.name), now, now);
  }

  static reconstitute(params: { id: string; email: string; name: string; createdAt: Date; updatedAt: Date }): User {
