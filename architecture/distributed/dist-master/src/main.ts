 ...
 *
 * 启动流程：
 * 1. fork 3 个 Worker 子进程（@dist/worker，各自监听不同端口）
 ...
 *
 * 分布式架构特征：
 * - 多进程：Master 和 Worker 是独立进程（对齐 java dist-* 的多服务部署）
 * - 网络通信：通过 HTTP 互调
...
async function bootstrap(): Promise<void> {
  ...
  // --- 1. 启动 Worker 子进程（@dist/worker 的入口脚本） ---
  console.log(`[Distributed] Starting ${WORKER_COUNT} worker processes...`);
  const workers: ChildProcess[] = [];
  const workerUrls: string[] = [];
  const workerScript = require.resolve("@dist/worker/worker");
  ...
  console.log("Workspace pkgs:  dist-shared-kernel / dist-task-core / dist-master / dist-worker");
